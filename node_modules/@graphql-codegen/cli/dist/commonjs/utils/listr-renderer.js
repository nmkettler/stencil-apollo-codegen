        ...
        if (err) {
            ...
            if (errorCount > 0) {
                ...
                const details = err.errors
                    ...
                    return { msg: core_1.isDetailedError(error) ? error.details : null, rawError: error };
                })
                    .map(({ msg, rawError }, i) => {
                    const source = err.errors[i].source;
                    msg = msg ? chalk_1.default.gray(indentString(common_tags_1.stripIndent(`${msg}`), 4)) : null;
                    const stack = rawError.stack ? chalk_1.default.gray(indentString(common_tags_1.stripIndent(rawError.stack), 4)) : null;
                    if (source) {
