import { isComplexPluginOutput, turnExtensionsIntoObjectTypes, federationSpec } from '@graphql-codegen/plugin-helpers';
...
export async function codegen(options) {
    ...
    if (options.schemaAst && documents.length > 0 && !skipDocumentValidation) {
        ...
        const errors = await validateGraphQlDocuments(options.schemaAst, [...documents, ...extraFragments.map((f) => ({ filePath: f.importFrom, content: { kind: Kind.DOCUMENT, definitions: [f.node] } }))]);
