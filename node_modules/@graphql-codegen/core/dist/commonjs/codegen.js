...
async function codegen(options) {
    ...
    if (options.schemaAst && documents.length > 0 && !skipDocumentValidation) {
        ...
        const errors = await graphql_toolkit_1.validateGraphQlDocuments(options.schemaAst, [...documents, ...extraFragments.map((f) => ({ filePath: f.importFrom, content: { kind: graphql_2.Kind.DOCUMENT, definitions: [f.node] } }))]);
